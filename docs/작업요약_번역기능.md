...
리팩터링 과정에서 "app/(모델링 엔진) ↔ backend/(서빙)" 계층 경계가 명확치 않은 코드가
반복 발견됨. 모델링에 속할 로직이 backend 서비스 함수에 섞여 있는 케이스들을 점검·정리한다.

- **확인된 사례 — 한국어 입력 검증**: `_contains_hangul`(비한국어 원문 차단)이
  `backend/services/translation_service.py` 의 `translate()` 안에 있음. 이는 "번역 모델이
  처리 가능한 입력인가"라는 모델링 판단이므로 app/translation 으로 이관 검토.
  - 책임 분리 원칙: 모델링(엔진)은 "처리 불가 여부 + 사유"를 반환하고,
    사용자 응답 포장(메시지/JSON 구조)은 backend(서빙층)가 담당.
  - 부수 효과: 현재 차단이 backend 에만 있어 파이프라인 직접 호출(검증 스크립트)로는
    TST-TRANS-001 예외 흐름(영어 차단)을 검증할 수 없음 — 이관 시 해소됨.
  - 더 근본적 재검토: 언어 적합성 검증은 "번역 시점"보다 **원문 업로드/저장 시점**에
    수행하는 것이 자연스러움(잘못된 데이터의 조기 차단). 1차 방어선=업로드,
    번역 단계는 보조 안전망으로 위치 재정의 고려.
- **추가 점검 대상**: 같은 패턴(모델링 로직이 backend 에 섞임)이 다른 기능에도 있는지 전수 점검.
  특히 아직 app/ 로 분리되지 않은 기능들(이미지·현지화 가이드 등)이 backend/services 에
  모델링과 서빙이 혼재된 채로 있으므로, app/ 구조로 옮길 때 함께 계층을 가른다.
