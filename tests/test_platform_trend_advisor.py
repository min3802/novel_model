...
import api_server
from data.localization_guide.platform_trend_advisor import build_localization_advice, rank_countries
from data.localization_guide.platform_trend_guide import load_trend_data
...
class PlatformTrendAdvisorTests(unittest.TestCase):
    ...
    def test_with_synopsis_recommends_country_then_generates_guide(self):
        ...
        self.assertIn(result["targetCountry"], {rec["country"] for rec in result["recommendedCountries"]})
        self.assertIn("title_synopsis_localization", result["sections"])
        self.assertIn("evidence_used", result["sections"])
        self.assertIn("\uc2dc\ub189\uc2dc\uc2a4 \uae30\ubc18", result["htmlReport"])
    ...
    def test_api_guide_preserves_required_legacy_html_sections(self):
        result = api_server.guide({"targetCountry": "\ubbf8\uad6d", "genre": "\ud310\ud0c0\uc9c0"})
