...
def find_ancestor_containing(start: Path, relative_path: str) -> Path:
    ...
    The original project used ``app/translation`` as the package anchor.  This
    test bundle may place the package directly under ``translation``.  The
    fallback keeps the runtime usable without changing the translation agents.
    """
...
def package_project_root(start: Path | None = None) -> Path:
    ...
    try:
        ...
        return find_ancestor_containing(anchor, "translation")
...
def repository_root(start: Path | None = None) -> Path:
    ...
    try:
        ...
        try:
            return find_ancestor_containing(anchor, "prompts/agent_runtime")
        except FileNotFoundError:
            return package_project_root(anchor)
